...
class Qwen2VLGRPOVLLMTrainerModified(Trainer):
    def __init__(
        ...
        if is_deepspeed_zero3_enabled():
            if "Qwen2-VL" in model_id:
                ...
                print("debug if is_deepspeed_zero3_enabled")
                with deepspeed.zero.Init(enabled=False):
                    self.ref_model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
                        model_id, torch_dtype=torch.bfloat16, **model_init_kwargs
                    )
            elif "Aria" in model_id:
        ...
        if processing_class is None:
            if "Qwen" in model_id or "Aria" in model_id:
                ...
                # processing_class.eos_token_id = processing_class.eos_token_id
                if "Qwen" in model_id:
        ...
        print("debug reward_processing_classes is None:")
        ...
        if self.use_vllm:
            ...
            if self.accelerator.is_main_process:
                ...
                with world_size_patch, profiling_patch:
                    ...
                    print("debug vllm is running on:")
                    self.llm = LLM(
                    ...
                    print("debug after self.llm = LLM:")
                self.sampling_params = SamplingParams(
                    temperature=args.temperature,
                    max_tokens=self.max_completion_length,
                )
                print("debug self.sampling_params")

            self._last_loaded_step = 0  # tag to avoid useless loading during grad accumulation
            print("debug self._last_loaded_step")

            # When using vLLM, the main process is responsible for loading the model weights. This can cause process
            # desynchronization and seems to lead to DeepSpeed hanging during initialization. To prevent this, we
            # synchronize all processes after vLLM has been fully initialized.
            # self.accelerator.wait_for_everyone()
            self.accelerator.print(f"Rank {self.accelerator.local_process_index} reached wait_for_everyone")
            self.accelerator.wait_for_everyone()
            self.accelerator.print(f"Rank {self.accelerator.local_process_index} passed wait_for_everyone")
            # print("debug accelerator.wait_for_everyone:")
        else:
            raise ValueError(
        ...
        if self.ref_model is not None:
            print("self.ref_model is not None:")
            if self.is_deepspeed_enabled:
                self.ref_model = prepare_deepspeed(self.ref_model, self.accelerator)
                # self.ref_model = self.accelerator.prepare_model(self.ref_model, evaluation_mode=True)
                # self.ref_model = self.ref_model.to(dtype=torch.bfloat16)
            else:
                self.ref_model = self.accelerator.prepare_model(self.ref_model, evaluation_mode=True)

        for i, reward_func in enumerate(self.reward_funcs):
            print("reward_func in enumerate(self.reward_funcs)")
            if isinstance(reward_func, PreTrainedModel):
    ...
    def _prepare_inputs(
        ...
        if self.args.use_vllm:
            ...
            # Log memory info every 10 steps
            if self.accelerator.is_main_process:
                vm = psutil.virtual_memory()
                print(f"[Step {self.state.global_step}] CPU RAM used: {(vm.total - vm.available) / 1024 ** 3:.2f} GB")
                print(f"[Step {self.state.global_step}] CUDA Memory Allocated: {torch.cuda.memory_allocated() / 1024 ** 3:.2f} GB")
                print(f"[Step {self.state.global_step}] CUDA Max Allocated: {torch.cuda.max_memory_allocated() / 1024 ** 3:.2f} GB")
                # gc.collect()
    ...
    def compute_loss(
        ...
        del prompt_ids, prompt_mask, completion_ids, completion_mask, input_ids, attention_mask
        del pixel_values, image_grid_thw, per_token_logps, ref_per_token_logps, advantages
        torch.cuda.empty_cache()
        gc.collect()
